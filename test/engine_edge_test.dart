...
void checkBounds(GameState s, String where) {
  ...
  for (final e in s.relations.entries) {
    expect(
      ...
      reason: '$where ${e.key} 호감',
    );
    expect(
      e.value.trust,
      inInclusiveRange(0, 100),
      reason: '$where ${e.key} 신뢰',
    );
  }
}
    ...
    for (final ev in plan) {
      if (ev.once) {
        expect(
          onceSeen.add(ev.id),
          isTrue,
          reason: '$where once 이벤트 재등장: ${ev.id}',
        );
        expect(
          s.seen,
          isNot(contains(ev.id)),
          reason: '$where seen 인데 또 계획됨: ${ev.id}',
        );
      }
...
void main() {
  ...
  GameState fresh({int seed = 1, int run = 1, int day = 1}) =>
      GameState.fresh(real.config, real.characters, seed: seed, run: run)
        ..day = day;
  ...
  group('스탯 클램프', () {
    ...
    test('변화가 없으면 delta 가 비어 있다', () {
      ...
      final d = applyEffects(
        s,
        const Effects(affection: {'seoyeon': -3}, stats: {Stat.talk: 0}),
      );
    ...
    test('아침 행동과 룰렛도 음수로 못 내려간다', () {
      ...
      final rest = EventEngine.rouletteSlots.indexWhere(
        (e) => e.$2.containsKey(Stat.stress) && e.$2[Stat.stress]! < 0,
      );
  ...
  group('100일 완주', () {
    ...
    for (final policy in Policy.values) {
      test(
        '${policy.name} 정책 · 시드 $seeds개: 예외 없음, 계획 중복 없음, once 재등장 없음, 엔딩 유효',
        () {
    ...
    test('같은 시드·같은 정책이면 결과가 완전히 같다(결정성)', () {
      for (var seed = 1; seed <= 5; seed++) {
        expect(
          simulate(real, seed, Policy.best),
          simulate(real, seed, Policy.best),
        );
      }
    });
        ...
        while (c.phase != Phase.ending) {
          ...
          switch (c.phase) {
            case Phase.action:
              ...
              final ok = await c.startDay(
                real.config.actions[pr.nextInt(real.config.actions.length)],
              );
              expect(ok, isTrue);
            case Phase.event:
              while (!c.linesDone) {
              ...
              if (c.lastOutcome == null) {
                // 되돌린 뒤엔 다시 고른다.
                c.choose(
                  pickIndex(Policy.random, c.choices, pr),
                  minigameSuccess:
                      c.current!.choices.any((x) => x.minigame != null)
                      ? true
                      : null,
                );
              }
  ...
  group('엔딩 60개 도달성', () {
    ...
    GameState stateFor(Ending e) {
      ...
      w.affection.forEach(
        (k, r) => s.rel(k == '*' ? e.character! : k).affection = pick(r, 100),
      );
      w.trust.forEach(
    ...
    test('immediate 엔딩은 immediate() 로도 잡힌다', () {
      ...
      expect(
        real.endings.where((e) => e.immediate).map((e) => e.id),
        unorderedEquals(['pickup_fall', 'burnout']),
      );
    });
  ...
  group('되돌리기', () {
    ...
    test(
      '실패 선택 → 되돌리기: stats/relations/flags/seen/album/combo/dayDelta 완전 원복',
      () async {
  ...
  group('하트', () {
    ...
    test('최대치를 넘지 않고, 가득 차면 타이머가 지금으로 맞춰진다', () {
      final e = EventEngine(mini);
      final s = GameState.fresh(mini.config, mini.characters, seed: 1, nowMs: 0)
        ..hearts = 0;
    ...
    test(
      '컨트롤러: 0개면 startDay 가 false, 시간이 지나면 다시 true, grantHeart 는 상한까지',
      () async {
        var now = 1_000_000;
  ...
  group('세이브', () {
    ...
    test('toJson 은 live 컬렉션을 공유하지 않는다', () {
      ...
      s.flags.add('q');
      expect(
        (j['stats'] as Map)[Stat.charm],
        real.config.initialStats[Stat.charm],
      );
  ...
  group('콤보', () {
    ...
    test('물오름 +20%p 가 확률 판정에 실제로 붙는다', () {
      final a = ms();
      final out1 = e.applyChoice(
        a,
        ev(),
        chance70(),
        random: FixedRandom([85, 99]),
      );
      expect(out1.success, isFalse);
      final b = ms()..combo = 3;
      final out2 = e.applyChoice(
        b,
        ev(),
        chance70(),
        random: FixedRandom([85, 99]),
  ...
  group('EndingResolver 동점·정렬', () {
    ...
    test('같은 우선순위면 호감도 높은 캐릭터', () {
      ...
      final r = EndingResolver([
        mk('h', 10, character: 'haneul'),
        mk('j', 10, character: 'jiwoo'),
        mk('d', 0, def: true),
      ]);
    ...
    test('캐릭터 없는 엔딩끼리 동점이면 파일 순서, 캐릭터 있는 쪽이 있으면 그쪽 우선', () {
      final s = fresh();
      final r1 = EndingResolver([
        mk('x', 10),
        mk('y', 10),
        mk('d', 0, def: true),
      ]);
      expect(r1.resolve(s).id, 'x');
      final r2 = EndingResolver([
        mk('y', 10),
        mk('x', 10),
        mk('d', 0, def: true),
      ]);
      expect(r2.resolve(s).id, 'y');
      final r3 = EndingResolver([
        mk('x', 10),
        mk('c', 10, character: 'seoyeon'),
        mk('d', 0, def: true),
      ]);
    ...
    test('아무것도 안 맞으면 default, default 없으면 마지막', () {
      ...
      expect(
        EndingResolver([
          mk('a', 5, when: never),
          mk('d', 0, def: true, when: never),
          mk('z', 1, when: never),
        ]).resolve(s).id,
        'd',
      );
      expect(
        EndingResolver([mk('a', 5, when: never), mk('z', 1, when: never)])
            .resolve(s)
            .id,
        'z',
      );
  ...
  group('StoryBundle.validate', () {
    ...
    test('lint: character 없는 이벤트의 * 사용을 잡는다', () {
      ...
      expect(dataLint(b), [
        'a.choices[0].effects: character 없이 * 사용 (효과가 버려짐)',
      ]);
    });
    ...
    test('실제 데이터의 lint 결과 (알려진 것만)', () {
      ...
      // 캐스트 6+6(docs/CAST_BIBLE.md)이 모두 들어와 역할 빈칸이 없다. 새로 비면 여기서 잡힌다.
      expect(real.castGaps, isEmpty);
      expect(
  ...
  group('난수 결정성', () {
    test('stableSeed 는 실행·플랫폼에 무관한 고정값', () {
      ...
      expect(
        EventEngine.stableSeed(1, 1, 'a'),
        isNot(EventEngine.stableSeed(1, 1, 'b')),
      );
      expect(
        EventEngine.stableSeed(1, 1, 'a'),
        isNot(EventEngine.stableSeed(1, 2, 'a')),
      );
      expect(
        EventEngine.stableSeed(1, 1, 'a'),
        isNot(EventEngine.stableSeed(2, 1, 'a')),
      );
    });
    ...
    test('세이브를 거쳐 복원한 상태도 같은 계획을 만든다', () {
      ...
      expect(
        engine.planDay(a).map((e) => e.id),
        engine.planDay(b).map((e) => e.id),
      );
      expect(
        engine.rng(a, 'x').nextInt(1 << 30),
        engine.rng(b, 'x').nextInt(1 << 30),
      );
    });
  ...
  group('하루 계획 세부', () {
    test('레이어 인덱스가 원본과 일치한다', () {
      ...
      expect(
        () => real.eventsByLayer[EventLayer.main]!.add(real.events.first),
    ...
    test('호감도 동률이면 시드에 따라 캐릭터가 갈리지만 결정적이다', () {
      ...
      for (var seed = 0; seed < 40; seed++) {
        ...
        final again = engine
            .planDay(fresh(seed: seed, day: 30))
            .firstWhere((e) => e.layer == EventLayer.route);
  ...
  test('호감이 오르지 않는 선택지는 크리티컬이 나지 않는다', () {
    final b = StoryBundle.fromJsonStrings(
      ...
      endings:
          '[{"id":"z","name":"z","tier":"bad","priority":1,"default":true}]',
    );
