...
/// 수정 전(route_a/b 원본) 측정치. 캐릭터 → 전략 → 마지막 루트 이벤트(r15)에 도달한 회차 비율.
/// focus 류는 그 캐릭터를 대상으로 한 회차만 센다. 1200시드, 미니게임 성공률 60%,
/// 2026-09-19 엔진 기준. 캐릭터마다 최소 한 전략에서 5%p 넘게 떨어지지 않아야 한다.
const baselineReach = <String, Map<String, double>>{
  'seoyeon': {'first': .018, 'maxAff': .448, 'focus': .860, 'focus+hint': .770, 'random': .035, 'doubleTimer': .273},
  'haneul': {'first': .176, 'maxAff': .427, 'focus': .890, 'focus+hint': .705, 'random': .094, 'doubleTimer': .329},
  'jiwoo': {'first': .018, 'maxAff': .007, 'focus': .200, 'focus+hint': .025, 'random': .092, 'doubleTimer': .018},
  'minjae': {'first': .035, 'maxAff': .056, 'focus': .675, 'focus+hint': .100, 'random': .142, 'doubleTimer': .167},
  'yeeun': {'first': .279, 'maxAff': .059, 'focus': .675, 'focus+hint': .170, 'random': .284, 'doubleTimer': .212},
  'doyun': {'first': .348, 'maxAff': .988, 'focus': .955, 'focus+hint': .895, 'random': .504, 'doubleTimer': .404},
};

/// 선택에 따라 갈리는 분기 플래그. 진행 플래그가 아니라서 "모든 경로에서 세워져야" 검사와
/// 막다른 길 검사에서 뺀다(그 분기를 안 고르면 뒤 이벤트를 못 보는 게 의도).
const branchFlags = {
  'yeeun_avoid_1', 'fake_record', 'overtraining', 'minjae_promised', 'minjae_met',
  'haneul_banmal', 'seoyeon_banmal', 'minjae_meet_asked', 'told_junho', 'taehyun_hint',
  'haneul_next_date', 'yeeun_avoid_2', 'yeeun_avoid_3', 'yeeun_letter_told',
};

/// day 창이 좁은 선택적 장면. 앞 이벤트를 놓치면 이 장면만 건너뛰고, 이 장면을 전제하는
/// 뒤 이벤트는 없다(orderOnly 로만 묶임). 그래서 막다른 길 검사에서 뺀다.
const optionalScenes = {'jiwoo_r02'};
  ...
  while (!engine.isFinished(s)) {
    ...
    while (queue.isNotEmpty) {
      ...
      for (final l in ev.lines) {
        if (l.isWait) s.stats[Stat.esteem] = (s.stat(Stat.esteem) - 1).clamp(0, 100);
      }
...
Set<String> mustPrecede(StoryBundle b, String id, [Map<String, Set<String>>? memo, Set<String>? stack]) {
  memo ??= {};
  stack ??= {};
  ...
  for (final m in b.events.where((m) => m.layer == EventLayer.main && m.day != null && m.trigger.flags.isEmpty)) {
...
bool precedesAny(StoryBundle b, String id, Set<String> alts, [Set<String>? stack]) {
  stack ??= {};
  ...
  for (final f in e.trigger.flags) {
    final xs = set[f] ?? {};
    if (xs.isNotEmpty && xs.every((x) => alts.contains(x) || precedesAny(b, x, alts, stack))) ok = true;
  }
...
bool cannotFollow(StoryBundle b, String later, String earlier) {
  ...
  for (final x in chain) {
    if (guaranteedFlags(b.eventById[x]!).any(e.trigger.notFlags.contains)) return true;
  }
  return false;
}
...
void main() {
  ...
  test('정적 검사: 뒤 이벤트 트리거가 앞 이벤트를 보장한다', () {
    ...
    for (final (later, earlier) in prerequisites) {
      ...
      final ok = alts.length == 1 ? mustPrecede(bundle, later, memo).contains(earlier) : precedesAny(bundle, later, alts);
      if (!ok) {
        final l = bundle.eventById[later]!;
        bad.add('$later ← $earlier (flags ${l.trigger.flags}, day ${l.trigger.day})');
      }
    }
    for (final (later, earlier) in orderOnly) {
      if (!cannotFollow(bundle, later, earlier)) bad.add('$later 뒤에 $earlier 가 나올 수 있음');
    }
  ...
  test('정적 검사: 막다른 길 없음 (앞 이벤트 상한이 뒤 이벤트 상한 이상)', () {
    ...
    List<String> issues(String later, String earlier, Set<String> alts) {
      ...
      if (e.layer != EventLayer.route || optionalScenes.contains(later)) return bad;
      // 뒤가 앞의 분기 플래그로만 이어지면(거짓 기록 → 발각 등) 진행 경로가 아니다.
      final link = l.trigger.flags.where((f) => set[f]?.contains(earlier) == true);
      ...
      for (final dim in ['affection', 'trust']) {
        ...
        em.forEach((k, r) {
          ...
          if (r.max < min(lMax, 100)) bad.add('$earlier $dim[$k] 상한 ${r.max} < $later 상한 $lMax');
        });
      }
      final eDayMax = e.trigger.day?.max ?? 100;
      final lDayMax = l.trigger.day?.max ?? 100;
      if (eDayMax < lDayMax) bad.add('$earlier day 상한 $eDayMax < $later day 상한 $lDayMax');
