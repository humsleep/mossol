...
- 구성(위→아래): AppBar(`D+N  ·  N장`) → 상태 줄(`HeartsRow` + `ComboBadge`) → `md`
  ...
  → `SectionHeader('오늘 아침에 뭘 할까')` + 행동 `AppListRow` 목록(사이 `listGap`).
...
- 관계 변화는 문장 나열 대신 `StatTile`(라벨 / 값 / 부호+변화량) 로 정렬한다.
  단, 표시 문자열은 `'서연 호감 +4'` 형태를 그대로 유지한다(테스트 고정).
...
- `MinigameScaffold` 가 구조를 전담한다: 제목 블록(패딩 `AppInsets.screenX` + 상단 `lg`)
  ...
  → 결과 블록.
- 결과 연출: 본문을 0.25 불투명도로 내리고 결과 배지를 띄운다. 배경색은
...
| 결과 패널 | `'크리티컬! 호감 2배'`, `'실패…'`, `'물올랐다!'`, `'결과'`, `'계속'` |
| 정산 | AppBar `'D+1 정산'`, `'서연 호감 +4'`, 스탯 변화 `'(+3)'` 포함 |
