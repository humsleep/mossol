...
- 그 칸에 `docs/store_screenshots/promo/01.png` ~ `08.png` 를 **순서대로** 끌어다 놓는다(1320×2868).
- 다른 크기(6.5형 등)는 비워 둔다 — 6.9형에서 자동으로 줄여 쓴다.
- **"스크린샷 크기는 1242 × 2688px … 이어야 합니다" 오류**가 나면 6.5형 칸에 올리고 있는 것이다. 탭을 6.9형으로 바꾼다.
  6.9형 칸이 아예 없으면 6.5형용 한 벌(1284×2778)이 `docs/store_screenshots/promo65/` 에 있다. 다시 만들려면 `python3 tool/store_images/make.py --65`.
